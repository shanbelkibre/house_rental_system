...
import { Spinner, Alert, Button, Badge, StarRating, Card, Modal, Input } from '../components/UI';
...
export default function HouseDetailPage() {
  ...
  const sendRequest = async () => {
    try {
      ...
      setSuccess('Rental request sent!'); setReqModal(false);
  ...
  const scheduleVisit = async () => {
    try {
      ...
      setSuccess('Visit scheduled!'); setVisitModal(false);
    } catch (e) { setError(e.response?.data?.message || 'Failed'); }
  };

  const submitReview = async () => {
    try {
      await api.post('/reviews', { house_id: id, rating: revRating, comment: revComment });
      setSuccess('Review submitted!'); setRevModal(false);
  ...
  if (loading) return <div className="flex justify-center py-20"><Spinner size={12} /></div>;

  if (error && !house) return (
    <div className="max-w-2xl mx-auto px-4 py-16 text-center">
      <Alert type="error" message={error} />
      <Button onClick={() => navigate('/houses')} variant="outline" className="mt-4">← Back to listings</Button>
    </div>
  );

  const images = house.images || [];
  const currentImg = images[imgIdx]?.image_path
    ? `http://localhost:8000/storage/${images[imgIdx].image_path}`
    : null;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="rounded-2xl overflow-hidden bg-gray-100 h-72 relative">
        {currentImg
          ? <img src={currentImg} alt={house.title} className="w-full h-full object-cover" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <Badge label={house.status === 'available' ? 'Available' : 'Rented'} color={house.status === 'available' ? 'green' : 'red'} />
          </div>

          <div className="flex items-center gap-4">
            <span className="text-3xl font-bold text-blue-600">ETB {Number(house.price).toLocaleString()}<span className="text-base font-normal text-gray-400">/mo</span></span>
            <span className="text-gray-500">🛏 {house.rooms} room{house.rooms !== 1 ? 's' : ''}</span>
            <span className="flex items-center gap-1"><StarRating value={Math.round(avgRating)} /><span className="text-sm text-gray-500">({avgRating})</span></span>
          </div>

          <Card>
            <h2 className="font-semibold text-gray-700 mb-2">Description</h2>
            <p className="text-gray-600 text-sm leading-relaxed whitespace-pre-wrap">{house.description}</p>
          </Card>
        ...
        <div className="space-y-4">
          <Card>
            ...
            {house.owner?.phone && <p className="text-sm text-gray-500 mt-1">📞 {house.owner.phone}</p>}
          </Card>
      ...
      <Modal open={visitModal} onClose={() => setVisitModal(false)} title="Schedule a Visit">
        <div className="space-y-4">
          <Input label="Visit Date & Time" type="datetime-local" value={visitDate} onChange={(e) => setVisitDate(e.target.value)} />
          <div className="flex gap-2 justify-end">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              {[1,2,3,4,5].map((n) => (
                <button key={n} onClick={() => setRevRating(n)} className={`text-2xl ${n <= revRating ? 'text-yellow-400' : 'text-gray-300'}`}>★</button>
              ))}
            </div>
          </div>
          <div>
