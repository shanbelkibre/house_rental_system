  ...
  return (
    <div className="min-h-screen bg-black text-white pt-24 pb-16">
      <div className="max-w-3xl mx-auto px-6 space-y-6">
        <div className="flex items-center justify-between border-b border-white/10 pb-6">
          <div>
            ...
            {unread > 0 && <p className="text-sm text-blue-400 font-medium mt-1">{unread} unread message{unread > 1 ? 's' : ''}</p>}
          </div>
          {unread > 0 && (
            <button 
              onClick={markAll}
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-gray-400 bg-gray-900/30 rounded-3xl border border-white/5">
          <div className="text-6xl mb-4">🔔</div>
          <p className="text-lg">You're all caught up!</p>
          <p className="text-sm mt-2 opacity-60">No new notifications at the moment.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {notifs.map((n) => (
            <div 
              ...
              className={`rounded-2xl border p-5 flex items-start gap-4 transition-all duration-200 cursor-pointer hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 ${n.is_read ? 'bg-gray-900/60 border-gray-800' : 'bg-blue-900/20 border-blue-500/30'}`}
            >
              <div className="mt-1 flex-shrink-0">
                <span className="text-2xl">{n.is_read ? '📭' : '📬'}</span>
              </div>
              <div className="flex-1">
                <h3 className={`font-medium mb-1 text-lg ${n.is_read ? 'text-gray-300' : 'text-white font-semibold'}`}>
        ...
        {selectedNotif && (
          <div className="space-y-5">
            <div className="flex items-center gap-4 border-b border-gray-100 pb-5">
              ...
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Received</p>
                <p className="text-sm font-medium text-gray-800">{new Date(selectedNotif.created_at).toLocaleString()}</p>
              </div>
            </div>
