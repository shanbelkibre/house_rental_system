...
export default function ProfilePage() {
  ...
  const changePassword = async (e) => {
    e.preventDefault();
    setChangingPw(true); setError(''); setSuccess('');
    try {
  ...
  const roleColor = { admin: 'text-purple-600', owner: 'text-blue-600', renter: 'text-green-600' };
    ...
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <Card className="flex items-center gap-4">
        <div className="w-20 h-20 rounded-full bg-gray-100 overflow-hidden flex-shrink-0">
          {user?.profile_image
            ? <img src={`http://localhost:8000/storage/${user.profile_image}`} alt="" className="w-full h-full object-cover" />
            : <div className="w-full h-full flex items-center justify-center text-3xl">👤</div>
          }
        </div>
        <div>
          <p className="text-xl font-bold text-gray-800">{user?.name}</p>
          <p className={`text-sm font-semibold capitalize ${roleColor[user?.role]}`}>{user?.role}</p>
          <p className="text-sm text-gray-500">{user?.email}</p>
          {user?.is_verified && <span className="text-xs text-green-600 font-medium">✓ Verified</span>}
        </div>
      </Card>
      ...
      <Card>
        ...
        <form onSubmit={changePassword} className="space-y-3">
          ...
          <Input label="Confirm New Password" type="password" value={pwForm.new_password_confirmation} onChange={setPw('new_password_confirmation')} required />
